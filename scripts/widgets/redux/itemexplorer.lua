...
function ItemExplorer:_DoInit(title_text, contained_items, list_options)
	...
	self.set_title = self.footer:AddChild(Text(HEADERFONT, 20))
    self.set_title:SetPosition(0,-25)
    self.set_title:SetColour(UICOLOURS.HIGHLIGHT_GOLD)
    self.set_title:SetHAlign(ANCHOR_RIGHT)
...
function ItemExplorer:RepositionFooter(new_parent, y, footer_width)
    if self.footer then
        ...
        self.set_title:SetRegionSize(footer_width, 30)
...
function ItemExplorer:_UpdateItemSetInfo(item_key)
	...
	if in_collection then
		...
		if IsItemIsReward(item_type) then
			self.set_title:SetString(STRINGS.SET_NAMES[item_type] .. " " .. STRINGS.UI.SKINSSCREEN.BONUS)
    ...
    elseif pack then
		...
		self.set_title:Show()
        self.set_title:SetString(STRINGS.SKIN_NAMES[pack])
