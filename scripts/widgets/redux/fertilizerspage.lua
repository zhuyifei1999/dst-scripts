...
local FertilizersPage = Class(Widget, function(self, parent_widget, ismodded)
	...
	for k, v in pairs(FERTILIZER_DEFS) do
		...
		if ((not self.ismodded) == (not v.modded)) then
			table.insert(fertilizer_grid_data, {fertilizer = k, def = v})
		end
	end

	table.sort(fertilizer_grid_data, function(a, b)
		a = a.def
		b = b.def
		local a_nutrient_type_count = 0
		local b_nutrient_type_count = 0
		local a_nutrient_type_total = 0
		local b_nutrient_type_total = 0
		for i = 1, 3 do
			if a.nutrients[i] > 0 then
				a_nutrient_type_count = a_nutrient_type_count + 1
				a_nutrient_type_total = a_nutrient_type_total + i
			end
			if b.nutrients[i] > 0 then
				b_nutrient_type_count = b_nutrient_type_count + 1
				b_nutrient_type_total = b_nutrient_type_total + i
			end
		end

		if a_nutrient_type_count ~= b_nutrient_type_count then
			return a_nutrient_type_count < b_nutrient_type_count
		end

		if a_nutrient_type_total ~= b_nutrient_type_total then
