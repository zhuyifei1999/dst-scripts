...
local function MakeItemWidget(root, cursor, x, y, ingredient_size, item_name, name_prefix)
    ...
    backing:SetHoverText(STRINGS.NAMES[string.upper(name_prefix..(item_name_remap[item_name] or item_name))] or STRINGS.UI.PLANTREGISTRY.NEEDSMORERESEARCH, hovertext_params)
...
local WeedPlantPage = Class(PlantPageWidget, function(self, plantspage, data)
    ...
    if self.known_percent >= LEARN_PERCENTS.EFFECTS then
        effects_y = effects_y - 10 - title_font_size / 2
        self.effects_text = self.root:AddChild(Text(HEADERFONT, title_font_size,
            STRINGS.UI.PLANTREGISTRY.EFFECTS[string.upper(self.data.plant)] or
            STRINGS.UI.PLANTREGISTRY.EFFECTS.NONE,
            PLANTREGISTRYUICOLOURS.UNLOCKEDBROWN))
