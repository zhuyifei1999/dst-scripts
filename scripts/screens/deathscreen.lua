...
local DeathScreen = Class(Screen, function(self, days_survived, start_xp, escaped, capped)
    ...
    self.character.isrespawning = true
...
function DeathScreen:MakeMenuItems(escaped)
    local menu_items =
    {
        {text = STRINGS.UI.DEATHSCREEN.DISCONNECT, cb = function() self:OnMenu(escaped) end}
    }

    if escaped then
        table.insert(menu_items,  {text = STRINGS.UI.DEATHSCREEN.CONTINUE, cb = function() self:OnContinue() end})
    else
        table.insert(menu_items,  {text = STRINGS.UI.DEATHSCREEN.CHARACTER, cb = function() self:OnChangeCharacter() end})
...
function DeathScreen:NotCapped(days_survived, start_xp, escaped)
    ...
    local menu_items = self:MakeMenuItems(escaped)
...
function DeathScreen:OnChangeCharacter()
    local function onSet(character, random)
        TheFrontEnd:PopScreen()
        if character then
            self.character.prefab = character
        end
    end

    TheFrontEnd:PushScreen(LobbyScreen(Profile, onSet, false, self.character.prefab))
end
...
function DeathScreen:OnRetry() 
    if not TheNet:IsDedicated() then
        local function doSpawn()
            TheFrontEnd:PopScreen()
            TheNet:SendSpawnRequestToServer(self.character.prefab, self.character.skin_name)
            if TheNet:GetIsServer() and not TheNet:IsDedicated() then
                assert(ThePlayer ~= nil, "could not spawn player character")
            end
        end
...
function DeathScreen:OnContinue()
 ...
 function DeathScreen:DoConfirmQuit()   
    self.active = false
