...
function MultiplayerMainScreen:DoInit()
    ...
    -- TEMP HACK FOR THE BETA VERSION STRING - REMOVE AFTER THE BETA
	self.updatename:SetString(subfmt(STRINGS.UI.MAINSCREEN.DST_FORGE_BETA_UPDATENAME, {version=APP_VERSION}))
...
function MultiplayerMainScreen:MakeMainMenu()
	...
	if not TheFrontEnd:GetIsOfflineMode() and not IsFestivalEventActive() then
    ...
    if IsFestivalEventActive() then
        local festival_button = MakeMainMenuButton(GetFestivalEventInfo().MENU,  function() self:OnFestivalEventButton() end, STRINGS.UI.MAINSCREEN.TOOLTIP_FESTIVALEVENT, self.tooltip)
