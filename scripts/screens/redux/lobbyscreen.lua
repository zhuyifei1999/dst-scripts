...
--	Settings.match_results.outcome = {won = true, round = 5, time = 333, total_deaths = 23}
	...
	self.panels = {}
	table.insert(self.panels, {panelfn = WxpPanel,				enabledfn = function() return (Settings.match_results.wxp_data ~= nil and Settings.match_results.wxp_data[TheNet:GetUserID()] ~= nil) or Settings.match_results.mvp_cards end})
	...
	if not TheNet:IsDedicated() then
		...
		if player_stats ~= nil then
			...
			TheSim:SetPersistentString("forge_stats/forge_stats_"..tostring(player_stats.session)..".csv", str)
			print(str)
		end
	end
end)
