...
local TextListPopupDialogScreen = Class(Screen, function(self, title, str, body, buttons, spacing)
            ...
            if string.len(str) == 1 then
                ...
                for i=1, string.len(str) do
                    ...
                    if (endPos ~= startPos and endPos > startPos) or (endPos == string.len(str)) then
                        if endPos < string.len(str) or (endPos == string.len(str) and string.sub(str, endPos, endPos) == ",") then endPos = endPos - 1 end
...
function TextListPopupDialogScreen:OnControl(control, down)
    ...
    if self.buttons and TheInput:ControllerAttached() then
        for i,v in ipairs(self.buttons) do
            if control == v.controller_control and not down then
                TheFrontEnd:GetSound():PlaySound("dontstarve/HUD/click_move")
                v.cb()
                return true
            end
