...
WORLD_SPECIAL_EVENT = SPECIAL_EVENTS.LAVAARENA
...
function GetFestivalEventInfo()
    -- TODO(dbriscoe): Get this from a server? Return nil if no event is active.
    return {
        MENU = STRINGS.UI.MAINSCREEN.FESTIVALEVENT,
        TITLE = STRINGS.UI.FESTIVALEVENTSCREEN.TITLE_LAVAARENA,
        EVENT_KEY = "lavaarena", -- use this to reference event-specific assets
        GAME_MODE = "lavaarena",
...
function GetActiveFestivalEventServerName()
    return GetFestivalEventInfo().SERVER_NAME
end

-- Refers to intermittent scheduled events.
function IsFestivalEventActive()
    return GetFestivalEventInfo() ~= nil
end
