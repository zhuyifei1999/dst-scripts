...
function GroundPounder:GroundPound_Offscreen(position)
	...
	for k,v in pairs(ents) do
		...
		if dsq < breakobjectsRadiusSQ then
			if v and self.destroyer and v.components.workable and v.components.workable.workleft > 0 and v.components.workable.action ~= ACTIONS.NET then
				v.components.workable:Destroy(self.inst)
			end
			if v and self.burner and v.components.burnable and not v:HasTag("fire") and not v:HasTag("burnt") then
