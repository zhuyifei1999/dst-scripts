...
function FrostyBreather:OnUpdate(dt)
	if not self.breath and TheWorld.state.temperature < TUNING.FROSTY_BREATH then
		self:Enable()
	elseif self.breath and TheWorld.state.temperature > TUNING.FROSTY_BREATH then
		self:Disable()
	end
end
