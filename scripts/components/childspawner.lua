...
function ChildSpawner:DoSpawnChild(target, prefab, radius)
    ...
    local offset = FindWalkableOffset(Vector3(x, 0, z), math.random() * PI * 2, (radius or self.spawnradius or .5) + self.inst:GetPhysicsRadius(0), 8, false, true, NoHoles, self.allowwater)
    if offset == nil then
        return
    end
