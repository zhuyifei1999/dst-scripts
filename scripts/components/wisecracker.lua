local Wisecracker = Class(function(self, inst)
    ...
    inst:ListenForEvent("heargrue", function(inst, data)
            if inst.components.talker then inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_CHARLIE")) end
    end)
    ...
    inst:ListenForEvent("ghostdelta", 
        function(inst, data) 
            ...
            inst.components.talker:Say(GetString(inst.prefab, "ANNOUNCE_GHOSTDRAIN"))
                end
            end    
        end)
	...
	if self.inst.LightWatcher:IsInLight() then
		if not self.inlight then
			if self.inst.LightWatcher:GetTimeInLight() >= light_thresh then
				self.inlight = true
				if self.inst.components.talker and not self.inst:HasTag("playerghost") then self.inst.components.talker:Say(GetString(self.inst.prefab, "ANNOUNCE_ENTER_LIGHT")) end
			end
		end
	else
		if self.inlight then
			if self.inst.LightWatcher:GetTimeInDark() >= dark_thresh then
				self.inlight = false
				if self.inst.components.talker then self.inst.components.talker:Say(GetString(self.inst.prefab, "ANNOUNCE_ENTER_DARK")) end
			end
		end
	end
end
