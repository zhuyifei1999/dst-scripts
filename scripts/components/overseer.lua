...
function Overseer:InitFight()
        ...
        self.data.kills         = {}
        self.data.caught        = {}
        self.data.sanity_start  = math.floor(ThePlayer.components.sanity:GetPercent()*100)
        self.data.hunger_start  = math.floor(ThePlayer.components.hunger:GetPercent()*100)
        self.data.health_start  = math.floor(ThePlayer.components.health:GetPercent()*100)
        self.data.health_abs    = math.floor(player.components.health.currenthealth)
        self.data.armor_absorbed = 0
        self.data.trod          = TheWorld.Map ~= nil and TheWorld.Map:GetNumVisitedTiles() or 0
...
function Overseer:Process()
    ...
    data.health_end     = math.floor(ThePlayer.components.health:GetPercent()*100)
    data.health_end_abs = math.floor(player.components.health.currenthealth)
    data.died           = player.components.health.currenthealth <= 0
    data.trod           = TheWorld.Map:GetNumVisitedTiles() - data.trod
