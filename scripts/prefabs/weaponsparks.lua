...
local function Piercing_PlaySparksAnim(proxy)
    local inst = CreateEntity()
    ...
    if parent ~= nil then
        ...
        inst.Transform:SetPosition(x1 + x, y1 + y, z1 + z)

        inst:DoTaskInTime(3 * FRAMES, Piercing_PlaySparksAnimBack, inst.Transform:GetRotation() + 180, x1 - x, y1 + y, z1 - z, proxy.flip:value() == 2 or proxy.flip:value() == 4)
    end
