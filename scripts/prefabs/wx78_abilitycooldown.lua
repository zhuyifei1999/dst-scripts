local function OnNameDirty(inst)
	if ThePlayer and ThePlayer.components.wx78_abilitycooldowns then
		ThePlayer.components.wx78_abilitycooldowns:RegisterAbilityCooldown(inst)
	end
end

local function GetAbilityName(inst)
...
local function InitAbilityCooldown(inst, abilityname, duration)
	inst._name:set(abilityname)
	inst._len:set(math.clamp(math.floor(duration * 10 + 0.5), 0, 65535))
end

local function RestartAbilityCooldown(inst, duration)
	inst._len:set(math.clamp(math.floor(duration * 10 + 0.5), 0, 65535))
	SetPercent(inst, 1)
end
...
local function fn()
	...
	--Network variables
	inst._name = net_hash(inst.GUID, "wx78_abilitycooldown._name", "namedirty")
	inst._pct = net_byte(inst.GUID, "wx78_abilitycooldown._pct", "pctdirty")
	inst._len = net_ushortint(inst.GUID, "wx78_abilitycooldown._len")

	inst._pct:set(180)

	--Common interface
	inst.GetAbilityName = GetAbilityName
	...
	--Server interface
	inst.InitAbilityCooldown = InitAbilityCooldown
	inst.RestartAbilityCooldown = RestartAbilityCooldown
...
return Prefab("wx78_abilitycooldown", fn)