...
local function OnHealthDelta(inst,data)
    local isBlossom = TheWorld.components.sisturnregistry and TheWorld.components.sisturnregistry:IsBlossom() or nil
    local threshold = TUNING.ABIGAIL_GESTALT_HIDE_THRESHOLD
    if isBlossom then
        threshold = threshold / 2 
    end
    if inst:HasTag("gestalt") and data.newpercent < threshold and not inst:HasTag("gestalt_hide") then
        if inst._undo_transparency_task then
            UndoTransparency(inst)
            inst._undo_transparency_task:Cancel()
            inst._undo_transparency_task = nil
        end
    ...
    if inst:HasTag("gestalt") and data.newpercent >= threshold and inst:HasTag("gestalt_hide") then
