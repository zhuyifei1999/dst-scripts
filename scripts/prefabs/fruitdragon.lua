...
local function OnSave(inst, data)
	...
	if GetRemainingNapTime(inst) > 0 then
		...
		data.time_awake = GetTime() - inst._wakeup_time
	end
	data.sleep_variance = inst.sleep_variance
end

local function OnLoad(inst, data)
	if data ~= nil then
		inst.sleep_variance = data.sleep_variance or 0
		if data.sleeping then
			inst._wakeup_time = -math.huge
		else
			inst._wakeup_time = GetTime() - (data.time_awake or 0)
		end
