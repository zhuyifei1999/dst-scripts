...
local function OnRechargeTick(inst, delta)
    inst._recharge = inst._recharge + delta
...
local function OnRechargeDirty(inst)
    ...
    if inst.recharge:value() < 180 then
        if inst.rechargetime:value() > 0 then
            local dt = inst.rechargetime:value() / 180
            inst._recharge = inst.recharge:value()
            inst._rechargetask =
                dt >= FRAMES and
                inst:DoPeriodicTask(dt, OnRechargeTick, nil, 1) or
                inst:DoPeriodicTask(FRAMES, OnRechargeTick, nil, 180 * FRAMES / inst.rechargetime:value())
        elseif inst.rechargetime:value() == 0 then
            inst._recharge = inst.recharge:value()
            inst._rechargetask = inst:DoPeriodicTask(0, OnRechargeTick, nil, 180)
        end
    end
end
