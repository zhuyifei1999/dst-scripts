...
local function onsleeptick(inst, sleeper)
    ...
    if sleeper.components.sanity ~= nil and sleeper.components.sanity:GetPercentWithPenalty() < 1 then
        sleeper.components.sanity:DoDelta(TUNING.SLEEP_SANITY_PER_TICK * .67, true)
    end

    if sleeper.components.temperature ~= nil and sleeper.components.temperature.current < TUNING.SLEEP_TARGET_TEMP_BEDROLL_STRAW then
        sleeper.components.temperature:SetTemperature(sleeper.components.temperature:GetCurrent() + (TUNING.SLEEP_TEMP_PER_TICK * .5))
    end
