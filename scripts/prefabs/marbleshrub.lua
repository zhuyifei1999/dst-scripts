...
local statedata =
{
	{ -- short
		...
		growanim    = "grow_old_to_short",
		growsound   = "dontstarve/common/together/marble_shrub/wilt_to_grow", 
		workleft    = TUNING.MARBLESHRUB_MINE_SMALL,
		loot        = function() return math.random() < 0.5 and {"marblebean"} or {"marblebean", "marble"} end,
	},
	{ -- normal
		...
		loot        = function() return math.random() < 0.5 and {"marble"} or {"marble", "marble"} end,
	},
	{ -- tall
		idleanim    = "idle_tall",
		hitanim     = "hit_tall", 
		...
		loot        = function() return math.random() < 0.5 and {"marble", "marble"} or {"marble", "marble", "marble"} end,
	},
	{ -- old
		idleanim    = "idle_old",
		hitanim     = "hit_old", 
		breakanim   = "mined_old",
		growanim    = "grow_tall_to_old",
		growsound   = "dontstarve/common/together/marble_shrub/wilt", 
		workleft    = TUNING.MARBLESHRUB_MINE_OLD,
		loot        = function() return {"marblebean"} end,
	},
}
...
local GROWTH_STAGES =
{
    ...
    {
        time = function(inst) return GetRandomWithVariance(TUNING.MARBLESHRUB_GROW_TIME[4].base, TUNING.MARBLESHRUB_GROW_TIME[4].random) end,
        fn = function(inst) SetGrowth(inst) end,
        growfn = function(inst) DoGrow(inst) end
    },
}
