...
local whitelight_str = 
{
	{radius = 4.5, falloff = .8, intensity = 0.8},
	{radius = 5.0, falloff = .8, intensity = 0.8},
	{radius = 5.25, falloff = .8, intensity = 0.8},
	{radius = 5.5, falloff = .75, intensity = 0.8},
}
local light_str = 
{
	{radius = 4.5, falloff = .85, intensity = 0.75},
	{radius = 5.0, falloff = .85, intensity = 0.75},
	{radius = 5.25, falloff = .8, intensity = 0.75},
	{radius = 5.5, falloff = .75, intensity = 0.75},
}
...
local function UpdateLightState(inst, skip_toggle_anims)
	...
	if num_batteries > 0 then
		if not inst.onlywhite then
			...
			local r = #inst.components.container:FindItems(function(item) return item.prefab == "red_cap" or item.prefab == MUSHTREE_SPORE_RED end)
			local g = #inst.components.container:FindItems(function(item) return item.prefab == "green_cap" or item.prefab == MUSHTREE_SPORE_GREEN end)
			local b = #inst.components.container:FindItems(function(item) return item.prefab == "blue_cap" or item.prefab == MUSHTREE_SPORE_BLUE end)
			...
			inst.Light:SetRadius(whitelight_str[num_batteries].radius)
			inst.Light:SetFalloff(whitelight_str[num_batteries].falloff)
			inst.Light:SetIntensity(whitelight_str[num_batteries].intensity)
		end
		...
		elseif toggling then
            ...
            inst.SoundEmitter:PlaySound("dontstarve/wilson/lantern_LP", "loop")
		elseif not inst.onlywhite then
