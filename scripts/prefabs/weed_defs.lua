...
-- 
-- growth states
--	small (sprout) - dirt and no dirt (dirt if on farm turf is so it looks like something in the farm)
--	med - no dirt
--  full - no dirt - blooming

-- weeds:
-- forgetmelots (common)
--   spreads quickly
--   harvest is forgetmelots flower
--   flowers in Crock-Pot for forgetmelots tea - warming drink
--   flowers spoil quickly
--   digging up has a 25% change to respawn nearby in 2 days

-- tillweed	(rare)
--   full state periodically spawns soil debris and tilled soil
--   has a fiddlehead type thing (like plantains) at full state
--   harvest fiddlehead to use for healing balm (+30hp?) And wormwood can use it

-- firenettle (common)
--   will overheat debuff player if they bump into it
--   harvest nettles, eating will cause same debuff as weed
--   wormwood is immune to the debuff
--   fireproof (maybe)

-- vicious ivy (rare)
--   only spreads far away
--   if harvesting anything nearby, it will put the player in a vine cage (like the bone cage) that deals damage until you fight your way out of it.
--   optional: if rotten plants near buy, it will cage nearest player (cooldown on this cage)
--   cannot be harvested
--   mounted players and wormwood cannot be caged
