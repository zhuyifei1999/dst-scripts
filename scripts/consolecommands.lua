...
function c_retrofitagainstthegrain()
	if TheWorld.components.sandstorms:RetrofitCheckIfWorldContainsOasis() == true then
		print ("Antlion Retrofit: The world already contains the Antlion and Oasis.")
		return
	end

	print ("Antlion Retrofit: Starting.")

	local playernode = TheWorld.topology.nodes[ThePlayer.components.areaaware.current_area]
	
	local entsonlake = TheSim:FindEntities(playernode.cent[1], 0, playernode.cent[2], 4)
	for k,v in ipairs(entsonlake) do
		if v.components.workable ~= nil then
			v.components.workable:Destroy(ThePlayer)
		end
	end
	for k,v in ipairs(entsonlake) do
		if v.components.workable ~= nil then
	...
	SpawnPrefab("antlion_spawner").Transform:SetPosition(playernode.cent[1], 0, playernode.cent[2])
	print ("Antlion Retrofit: Antlion Spawn Point Added.")

    local num_sandstorm_nodes = 1
    local oasisnode = nil
    local node_edges = playernode.validedges
	table.insert(playernode.tags, "sandstorm")
    for _, edge_index in ipairs(node_edges) do
        local edge_nodes = TheWorld.topology.edgeToNodes[edge_index]
        local other_node_index = edge_nodes[1] ~= ThePlayer.components.areaaware.current_area and edge_nodes[1] or edge_nodes[2]

        if not table.contains(TheWorld.topology.nodes[other_node_index].tags, "sandstorm") then
			table.insert(TheWorld.topology.nodes[other_node_index].tags, "sandstorm")
			num_sandstorm_nodes = num_sandstorm_nodes + 1
			oasisnode = other_node_index
		end
    end

	print ("Antlion Retrofit: Nodes Converted to Sandstorms:", #node_edges)

	if oasisnode == nil then
		print ("Antlion Retrofit: Place for Oasis Lake Not Found!")
	else
		local oasislake_xz = TheWorld.topology.nodes[oasisnode].cent
		local entsonlake = TheSim:FindEntities(oasislake_xz[1], 0, oasislake_xz[2], 4)
		print ("Removing Objects for lake", #entsonlake)
		for k,v in ipairs(entsonlake) do
			if v.components.workable ~= nil then
				v.components.workable:Destroy(ThePlayer)
			end
		...
		SpawnPrefab("oasislake").Transform:SetPosition(oasislake_xz[1], 0, oasislake_xz[2])
		
		print ("Antlion Retrofit: Oasis Lake Added:", #node_edges)
	end
	
	print ("Antlion Retrofit: Done.")
end