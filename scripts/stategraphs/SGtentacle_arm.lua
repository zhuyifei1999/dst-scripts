...
local function EmergeCheck(inst)
    ...
    elseif inst:IsNearPlayer(TUNING.TENTACLE_PILLAR_ARM_STOPATTACK_DIST) then
...
local states=
{
    State{
        ...
        ontimeout = function(inst)
            local dist2 = inst:GetDistanceSqToClosestPlayer()
            if dist2 < EMERGE_MIN2 or (dist2 < EMERGE_MAX2 and math.random() > 0.2) then
