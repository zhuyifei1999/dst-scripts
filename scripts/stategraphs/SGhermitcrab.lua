...
local events =
{
    ...
    EventHandler("consumehealthcost", function(inst, data)
        if not inst.sg:HasStateTag("nocraftinginterrupt") then
    ...
    EventHandler("onsink", function(inst, data)
        if not inst.sg:HasStateTag("drowning") and
    ...
    EventHandler("yawn", 
        function(inst, data)
            ...
            --       frozen
            --       pinned
            if inst.sg:HasStateTag("sleeping") or
...
local states =
{
        ...
        timeline =
        {
            TimeEvent(24 * FRAMES, function(inst)
                if inst.sg.statemem.target ~= nil and
                    ...
                    inst.components.npc_talker:Say(GetString(inst, STRINGS.HERMITCRAB_ANNOUNCE_ROYALTY))
                else
    ...
    State
    {
        ...
        onenter = function(inst)
            ...
            if not inst:HasTag("fastbuilder") then
                ...
                if inst.components.talker ~= nil then
                    ...
                    if slow then
                        ...
                        if (inst.sg.mem.hungryslowbuildtalktime or 0) < t then
                            ...
                            inst.dotalkingtimers(inst)
                            inst.components.npc_talker:Say(GetString(inst, "ANNOUNCE_HUNGRY_SLOWBUILD"))
                        end
                    else
                        ...
                        elseif inst.sg.mem.hungryfastbuildtalktime < t then
                            inst.sg.mem.hungryfastbuildtalktime = nil
                            inst.dotalkingtimers(inst)
                            inst.components.npc_talker:Say(GetString(inst, "ANNOUNCE_HUNGRY_FASTBUILD"))
                        end
                    end
                end
    ...
    State
    {
        ...
        timeline =
        {
            ...
            TimeEvent(.7, function(inst)
                if inst.bufferedaction ~= nil and
                    ...
                    inst.dotalkingtimers(inst)
                    inst.components.npc_talker:Say(GetString(inst, "ANNOUNCE_DESPAWN"))
                end
            end),
        },
        ...
        {
            EventHandler("ontalk", function(inst)
                if not inst.AnimState:IsCurrentAnimation("channel_dial_loop")  then
    ...
    State{
        ...
        onenter = function(inst)
            inst.components.locomotor:Stop()
            inst.AnimState:PlayAnimation("strum")
    ...
    State{
        ...
        events =
        {
            EventHandler("animover", function(inst)
                if inst.AnimState:AnimDone() then
                    inst.dotalkingtimers(inst)
                    inst.components.npc_talker:Say(GetString(inst, "ANNOUNCE_WASHED_ASHORE"))
...
--            TimeEvent(2*FRAMES, function(inst) inst:PerformBufferedAction() end),
    ...
    State{
        ...
        {
            TimeEvent(7 * FRAMES, function(inst)
                inst.sg.statemem.stafffx = SpawnPrefab("cointosscastfx")
                inst.sg.statemem.stafffx.entity:SetParent(inst.entity)
                inst.sg.statemem.stafffx.Transform:SetRotation(inst.Transform:GetRotation())
